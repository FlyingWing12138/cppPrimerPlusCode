...
double calculate(double x, double y, double (*ptFunc)(double, double));
double add(double x, double y);
...
int main()
{
    ...
    const char* operations[] = {"Addition", "Substraction", "Multiplication"};
    double x, y;

    while(true)
    {
        cout << "Enter 2 numbers, or other characters to exit: ";
        if (!(cin >> x >> y))
            break;
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            cout << operations[i] << " result: " << x << " and " << y << " resulted in: " << result << endl;
        }
    }
